<script lang="ts">
  ...
  let isExportActive = $state(false);
  ...
  $effect(() => {
    ...
    isExportActive = useStore.processedImages.length > 0;
  });
...
<div class="toolbox">
  ...
  <section class="btns">
    <button
      id="process"
      class:active={isProcessActive && !useStore.processState.isProcessing}
      disabled={!isProcessActive || useStore.processState.isProcessing}
      onclick={() => submitProcessData()}
    >
      {useStore.processState.isProcessing ? "Processing..." : "Process"}
    </button>
    <button
      id="export"
      class:active={isExportActive}
      disabled={!isExportActive}
      onclick={() => exportChannels(useStore.exportState.exportType.toString())}
    >
      Export
    </button>
  </section>
